...
class DoublyLinkedList:
    ...
    def remove(self, index):
        ...
        # Jos lista koko on 0 niin nostetaan errori
        if self.__len__() == 0:
            raise(ValueError('Index out of bounds'))
        ...
        # mennään haluttuun indeksiin asti
        for _ in range(index):
            previous_node = next_node
            next_node = next_node.next

        # Kun ollaan headissa asetetaan seuraava alkio uudeksi headiks
        if previous_node == None:
            self._head = next_node.next
            
        else:
            # Asetetaan nykyisen alkion tilalle sitä seuraava alkio
            # Eli ns ylikirjoitetaan haluttu alkio seuraavaksi alkioksi listassa
